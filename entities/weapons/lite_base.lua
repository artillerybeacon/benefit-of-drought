...
function SWEP:GetHeadshotMultiplier()
	if not self.HeadshotMultiplier then return 2.4 end
...
function SWEP:CanIronsight()
	return false // not self:IsSprinting() and not self:GetReloading() and self.Owner:IsOnGround()
end
...
function SWEP:HackySetDelay(delay)
